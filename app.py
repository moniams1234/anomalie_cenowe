...
<style>
/* ─────────────────────────────────────────────────────────────
   ELEGANCKI SZARY DESIGN — Anomalia Cenowa
   Cel: spokojny, czytelny dashboard, mniej agresywny niż granatowy UI
   ───────────────────────────────────────────────────────────── */
...
.main .block-container {
    background: #F3F4F6;
    padding: 1.6rem 2rem 2.5rem 2rem !important;
    max-width: 1480px;
}

/* Sidebar — szary, spokojny, elegancki */
...
.kpi-card:hover {
    ...
    border-color: #D1D5DB;
    box-shadow: 0 2px 8px rgba(17,24,39,0.06), 0 14px 32px rgba(17,24,39,0.06);
}
...
/* Small labels used in sidebar HTML */
.nav-item {
    padding: 8px 14px;
    border-radius: 10px;
    cursor: pointer;
...
/* Better spacing below columns */
div[data-testid="column"] {
    padding-bottom: 0.15rem;
}

/* Reduce visual clutter in sidebar session id */
...
with st.sidebar:
    ...
    st.markdown("<p style='font-size:12px;color:#E5E7EB;margin-top:-8px;'>Wykrywanie anomalii cenowych w danych magazynowych</p>", unsafe_allow_html=True)
    st.markdown("---")

    # Navigation
    st.markdown("<p style='font-size:11px;font-weight:700;color:#D1D5DB;letter-spacing:1px;margin-bottom:6px;'>NAWIGACJA</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<p style='font-size:11px;font-weight:700;color:#D1D5DB;letter-spacing:1px;margin-bottom:6px;'>USTAWIENIA ANALIZY</p>", unsafe_allow_html=True)
    threshold = st.slider(
        "Próg odchylenia (%)",
...
<p style="font-size:11px;font-weight:700;color:#D1D5DB;margin:0 0 8px 0;letter-spacing:1px;">JAK ZACZĄĆ</p>
<p style="font-size:11px;color:#E5E7EB;margin:4px 0;">1️⃣ Wgraj plik XLSX</p>
<p style="font-size:11px;color:#E5E7EB;margin:4px 0;">2️⃣ Ustaw próg odchylenia</p>
<p style="font-size:11px;color:#E5E7EB;margin:4px 0;">3️⃣ Kliknij "Analizuj"</p>
<p style="font-size:11px;color:#E5E7EB;margin:4px 0;">4️⃣ Opcjonalnie koryguj ceny</p>
<p style="font-size:11px;color:#E5E7EB;margin:4px 0;">5️⃣ Pobierz XLSX / PDF</p>
</div>
""", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='font-size:10px;color:#CBD5E1;text-align:center;margin-top:16px;'>Sesja: {st.session_state.session_id}</p>", unsafe_allow_html=True)
...
<div class="page-header">
  ...
  <p>Ręcznie ustaw ceny referencyjne dla wybranych indeksów z anomaliami</p>
    ...
    for _, row in anom_idx.iterrows():
        ...
        with c1:
            st.markdown(f"<p style='font-size:13px;font-weight:700;color:#111827;margin:8px 0;'>{idx_mat}</p>", unsafe_allow_html=True)
        with c2:
