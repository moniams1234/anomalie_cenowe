...
def generate_xlsx(
    ...
    _build_dane_sheet(wb, df_result, threshold_pct)
    _build_anomalie_sheet(wb, df_result)
    _build_zero_values_sheet(wb, df_result)
...
def _build_dane_sheet(wb: Workbook, df: pd.DataFrame, threshold_pct: float):
    ...
    for row_idx, (_, row) in enumerate(df[available].iterrows(), 2):
        for col_idx, col_name in enumerate(available, 1):
            ...
            if col_name == "Cena":
                # Excel formula
                cell.value = f"=IFERROR({war_col}{row_idx}/{stan_col}{row_idx},\"\")"
                cell.number_format = "#,##0.0000"
            elif col_name == "Odchylenie % do mediany":
                cell.value = f"=IFERROR(IF({ref_col}{row_idx}<>0,ABS({cena_col}{row_idx}-{ref_col}{row_idx})/{ref_col}{row_idx}*100,\"\"),\"\")"
                cell.number_format = "#,##0.00"
            elif col_name == "Anomalia":
                cell.value = f'=IFERROR(IF({dev_col}{row_idx}>={threshold_pct},"TAK","NIE"),"NIE")'
...
def _build_zero_values_sheet(wb: Workbook, df: pd.DataFrame):
    """Build sheet with rows where warehouse value equals zero."""
    ws = wb.create_sheet("Zerowe wartości")

    if "Wartość mag." not in df.columns:
        ws["A1"] = "Brak kolumny 'Wartość mag.' w danych"
        ws["A1"].font = Font(name="Arial", bold=True, color=CLR_ANOMALY_FG, size=11)
        return

    values_numeric = pd.to_numeric(df["Wartość mag."], errors="coerce")
    zero_df = df[values_numeric == 0].copy()

    if zero_df.empty:
        ws["A1"] = "Brak pozycji o wartości magazynu równej zero"
        ws["A1"].font = Font(name="Arial", bold=True, color=CLR_OK_FG, size=11)
        return
    ...
    available = [c for c in COLS if c in zero_df.columns]
    ...
    for row_idx, (_, row) in enumerate(zero_df[available].iterrows(), 2):
        for col_idx, col_name in enumerate(available, 1):
            val = row[col_name]
            cell = ws.cell(row=row_idx, column=col_idx)
            ...
            if col_name == "Data przyjęcia":
                ...
                "Stan mag.", "Wartość mag.", "Cena", "Mediana ceny indeksu",
                "Cena referencyjna", "Odchylenie % do mediany"
            ):
                cell.value = float(val) if pd.notna(val) else None
                cell.number_format = "#,##0.0000" if col_name != "Odchylenie % do mediany" else "#,##0.00"
            elif col_name == "Anomalia":
                cell.value = "TAK" if bool(val) else "NIE"
            else:
                cell.value = str(val) if pd.notna(val) else None

            cell.fill = PatternFill("solid", start_color="FFF7D6")
            cell.border = _thin_border()
            cell.font = Font(name="Arial", size=9)
            cell.alignment = Alignment(vertical="center")
...
def _build_settings_sheet(wb: Workbook, threshold_pct: float, file_name: str, df: pd.DataFrame):
    ...
    data = [
        ...
        ("Liczba pozycji z wartością magazynu 0", int((pd.to_numeric(df["Wartość mag."], errors="coerce") == 0).sum()) if "Wartość mag." in df.columns else 0),
